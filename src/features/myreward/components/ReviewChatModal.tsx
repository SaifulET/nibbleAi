...
export default function ReviewChatModal({ itemName, onClose, onSubmit }: ReviewChatModalProps) {
  const [step, setStep] = useState(0);
  const [messages, setMessages] = useState<{ sender: "bot" | "user"; text: string }[]>([
    { sender: "bot", text: `Hey there! 🍿 How did you like the ${itemName}?` },
  ]);
  ...
  const handleFinishChat = (replyText: string) => {
    const userAns1 = messages[1]?.text || `The ${itemName} was delicious, and the whole bag tasted super fresh.`;
    ...
    setMessages((prev) => [...prev, { sender: "user", text: replyText }]);
    setTimeout(() => {
      setStep(3); // show summary and first-time purchase
    }, 600);
  };
  ...
  const handleSendCustomReply = () => {
    ...
    if (step === 0) {
      handleUserReply(textToSend, "Glad to hear it!! What did you like most—taste, texture, or ingredients?");
    } else if (step === 1) {
      handleUserReply(textToSend, "Would you purchase it again or recommend to others?");
    } else if (step === 2) {
      ...
      <div className="w-full max-w-[500px] h-[80vh] min-h-[550px] bg-white rounded-[24px] shadow-2xl flex flex-col overflow-hidden relative border border-gray-100">
        ...
        <div className="px-6 py-4 bg-gray-50 border-b border-gray-100 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-semibold text-gray-800 truncate max-w-[280px]">Review: {itemName}</span>
          </div>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-2xl font-bold cursor-pointer">&times;</button>
        </div>

        {/* Chat message space */}
        <div className="flex-grow p-4 overflow-y-auto flex flex-col gap-3 bg-gray-50/50">
          {messages.map((m, idx) => (
            <div key={idx} className={`flex ${m.sender === "user" ? "justify-end" : "justify-start"} w-full`}>
              <div className={`max-w-[75%] rounded-[14px] p-3 text-sm font-medium shadow-sm ${
                m.sender === "user" ? "bg-[#3E3EDF] text-white" : "bg-[#F0F8FB] text-[#1F1D1D]"
              }`}>
                {m.text}
              </div>
            </div>
          ...
          {/* Step 3: Summary and First time selection */}
          {step >= 3 && (
            <div className="w-full flex flex-col gap-3 mt-2 animate-fade-in">
              <div className="bg-white border border-gray-200 rounded-[12px] p-4 shadow-sm relative">
                {isEditing ? (
                  ...
                  <p className="text-sm font-normal leading-relaxed text-gray-800">{reviewSummary}</p>
                )}
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className={`flex-1 py-2 text-sm border rounded-lg transition-all cursor-pointer font-medium ${
                      isFirstTime === true ? "bg-blue-50 border-[#3E3EDF] text-[#3E3EDF]" : "bg-white border-gray-300 text-gray-700"
                    }`}
                  >
                    Yes
                  </button>
                  <button
                    ...
                    className={`flex-1 py-2 text-sm border rounded-lg transition-all cursor-pointer font-medium ${
                      isFirstTime === false ? "bg-blue-50 border-[#3E3EDF] text-[#3E3EDF]" : "bg-white border-gray-300 text-gray-700"
                    }`}
                  >
                    No
                  </button>
        ...
        <div className="p-4 border-t border-gray-100 bg-white">
          {step < 3 ? (
            ...
            <button
              onClick={() => onSubmit(reviewSummary)}
              className="w-full py-3 bg-[#3E3EDF] text-white text-sm font-semibold rounded-lg hover:bg-[#3232c7] active:scale-[0.98] transition-all cursor-pointer shadow-md"
            >
              Submit Review &amp; Get $1.00
            </button>
          )}
        </div>
      </div>
